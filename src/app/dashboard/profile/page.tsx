            ...
            {championships.length > 0 && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        ...
                        {comboEnabledForChamp && <StatCard title="Combo (Dourada)" value={filteredStats.combo} icon={<Gem className="h-4 w-4" />} color="bg-yellow-500" description="Bucha + Gols da Ficha" link={`/dashboard/history?championship=${selectedChampionship}&user=${user?.id}&type=combo`} />}
                        {comboEnabledForChamp && <StatCard title="Bônus (Prata)" value={filteredStats.bonus} icon={<Trophy className="h-4 w-4" />} color="bg-slate-300" text="text-slate-900" description="Acerto exato dos Gols da Ficha" link={`/dashboard/history?championship=${selectedChampionship}&user=${user?.id}&type=bonus`} />}
